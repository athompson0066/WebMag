...
const CoverPage: React.FC<CoverPageProps> = ({ onStart, onStartAtSlide, config, slides }) => {
  const {
    ...
    sidebarBodyFontWeight, sidebarBodyItalic, sidebarFontColor, bodyFontSize
  } = config;
  ...
  const sidebarTitleStyle = {
    fontFamily: `"${activeSidebarTitleFont}", serif`,
    fontWeight: getFontWeight(sidebarTitleFontWeight),
    fontStyle: sidebarTitleItalic ? 'italic' : 'normal',
    color: activeSidebarFontColor
  };

  const sidebarBodyStyle = {
    fontFamily: `"${activeSidebarBodyFont}", sans-serif`,
    fontWeight: getFontWeight(sidebarBodyFontWeight),
    fontStyle: sidebarBodyItalic ? 'italic' : 'normal',
    color: activeSidebarFontColor
  };
  ...
  const LayoutBrutalist = () => (
    <div className="flex flex-col items-start justify-center h-full p-8 md:p-24 bg-white text-black overflow-hidden relative">
      <VideoBackground />
      <BackgroundImage className="opacity-30 filter grayscale" />
      <div className="relative z-10 w-full max-w-5xl">
        ...
        <p className="text-lg md:text-2xl font-light italic text-zinc-600 max-w-2xl mb-8 md:mb-12 leading-relaxed" style={bodyStyle}>{subtitle}</p>
        <StartButton />
      </div>
    </div>
  );
  ...
  const LayoutMinimal = () => (
    <div className="relative flex flex-col items-start justify-center h-full text-left p-8 md:p-32 bg-zinc-950 overflow-hidden">
      <VideoBackground />
      ...
      <div className="max-w-4xl relative z-10">
        ...
        <div className="w-16 md:w-24 h-[1px] bg-white/20 mb-8 md:mb-12"></div>
        <p className="text-lg md:text-2xl text-zinc-400 mb-10 md:mb-16 italic font-light leading-relaxed max-w-2xl" style={bodyStyle}>{subtitle}</p>
        <StartButton />
      </div>
    </div>
  );
  ...
  const LayoutClassic = () => (
    <div className="h-full flex flex-col p-8 md:p-24 bg-[#0a0a0a] relative overflow-hidden">
      <VideoBackground />
      <BackgroundImage className="opacity-60 scale-110" />
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="max-w-4xl py-10 md:py-20">
          ...
          <p className="text-lg md:text-2xl text-zinc-400 italic mb-8 md:mb-12 font-light" style={bodyStyle}>{subtitle}</p>
          <StartButton />
        </div>
      </div>
    </div>
  ...
  const LayoutGradient = () => (
    <div className="relative h-full flex flex-col items-start justify-center overflow-hidden bg-black p-8 md:p-32">
      <VideoBackground />
      <BackgroundImage className="opacity-50 scale-125 rotate-3" />
      ...
      <div className="relative z-10 text-left max-w-5xl">
        ...
        <p className="text-lg md:text-2xl text-zinc-400 italic mb-10 md:mb-16 font-light max-w-2xl" style={bodyStyle}>{subtitle}</p>
        <StartButton />
      </div>
    </div>
  );
  ...
  const LayoutGrid = () => (
    <div className="h-full bg-[#080808] flex flex-col p-8 md:p-24 relative overflow-hidden">
      <VideoBackground />
      <BackgroundImage className="opacity-30 filter grayscale" />
  ...
  const LayoutHero = () => (
    <div className="relative h-full flex items-center justify-start p-8 md:p-32 bg-black overflow-hidden">
      <VideoBackground />
      ...
      <div className="relative z-10 max-w-4xl text-left">
        ...
        <p className="text-lg md:text-2xl text-zinc-300 mb-10 md:mb-16 font-light italic border-l-[2px] md:border-l-[3px] border-white/20 pl-6 md:pl-12 max-w-2xl leading-relaxed" style={bodyStyle}>{subtitle}</p>
        <StartButton />
      </div>
    </div>
  );
  ...
  return (
    <div className="w-screen h-screen flex flex-col lg:flex-row bg-[#050505] overflow-y-auto lg:overflow-hidden">
      {/* Magazine Cover Left */}
      <div className="relative w-full lg:w-[65%] h-[70vh] sm:h-[80vh] lg:h-full overflow-hidden shadow-[30px_0_60px_rgba(0,0,0,0.5)] z-20 flex-shrink-0">
        {renderLayout()}
      </div>

      {/* Editorial Index Right */}
      <div className="w-full lg:w-[35%] h-auto lg:h-full z-10 p-8 md:p-20 bg-[#050505] overflow-y-auto no-scrollbar border-l border-white/5">
        <header className="mb-10 md:mb-16 border-b border-white/10 pb-8 md:pb-12">
          <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.5em] text-zinc-600 block mb-4" style={sidebarBodyStyle}>Table of Contents</span>
          <h4 className="text-3xl md:text-5xl font-serif italic text-white tracking-tighter" style={sidebarTitleStyle}>In This Issue.</h4>
        </header>
            ...
            >
              ...
              <div className="flex-1 border-b border-white/5 pb-6 md:pb-8 group-last:border-none">
                <div className="flex justify-between items-center mb-2 md:mb-3">
                  <h5 className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-zinc-500 group-hover:text-white transition-colors" style={sidebarBodyStyle}>{s.category}</h5>
                  <span className="text-[8px] font-mono text-white/20 uppercase tracking-widest" style={sidebarBodyStyle}>{s.type === 'external' ? 'Link' : 'Studio'}</span>
                </div>
                <h6 className="text-xl md:text-2xl font-serif font-bold text-white mb-2 md:mb-3 group-hover:italic transition-all" style={sidebarTitleStyle}>{s.title}</h6>
                <p className="text-[9px] md:text-[10px] text-zinc-600 leading-relaxed italic line-clamp-2 uppercase tracking-wider font-light" style={sidebarBodyStyle}>{s.description}</p>
              </div>
            </button>
          ))}
        </div>
